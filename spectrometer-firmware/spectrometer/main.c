...
////    UCB0IE |= UCRXIE;                         // Enable USCI_A0 RX interrupt
    ...
    TA1CCR1 = 25000-1;                        // 8 MHz / 8 / 50000 / 2 = 10 Hz
    TA1CCR0 = 25000-1;                        // Set CCR0 = CCR1 as counts up to CCR0
...
//    ADC12CTL0 |= ADC12ENC;                        // Enable conversions
    ...
    while(1)
    {
    	...
    	{
    		k = sprintf(charBuffer, "%d,", spectroBuffer[i]);
    		for(j = 0; j < k; j++)
    		{
//    			while(uartBusy);
//    			uartBusy = 1;
    			while (!(UCA0IFG&UCTXIFG));             // USCI_A0 TX buffer ready?
    			__delay_cycles(800); // delay 0.00001s at 8MHz
